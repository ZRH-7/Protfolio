...
import FrofilePic from '../../public/images/profile/developer-1.png'
		...
		<>
			...
			<main className='flex items-center text-dark w-full min-h-screen dark:text-light'>
					...
					<div className="flex items-center justify-between w-full lg:flex-col">
						<div className='w-1/2'>
							<Image
								src={FrofilePic}
								alt="tCoderBD"
								className='w-10/12 h-auto md:inline-block md:w-full'
								priority
						...
						<div className='w-1/2 flex-col items-center self-center lg:w-full lg:text-center'>
							...
							<div className='flex items-center self-start mt-6 lg:justify-center'>
								<Link href="/dummy.pdf" target='_blank' className='flex items-center bg-dark text-light p-2.5 px-7 rounded-lg text-lg font-semibold border-2 border-solid border-transparent hover:bg-light hover:text-dark hover:border-dark dark:bg-light dark:text-dark dark:hover:text-light dark:hover:bg-dark dark:hover:border-light md:p-2 md:px-4 md:text-base' download={true}>Resume <LinkArrow className={'w-6 ml-1'} /></Link>
								<Link href={'mailto:touhidul.sadeek@gmail.com'} target='_blank' className='ml-4 text-lg font-medium capitalize text-dark underline hover:no-underline dark:text-light md:text-base'>Contact</Link>
							</div>
						</div>
					</div>
				</Layout>
				...
				<div className='absolute right-8 bottom-8 inline-block w-24 lg:hidden'>
					<Image src={LightBulb} alt="tCoderBD" className='w-full h-auto' />
				</div>
			</main>
		</>
	)
